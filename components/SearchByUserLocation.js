...
export default function SearchByUserLocation({
  ...
  return (
    <>
      <button
        disabled={loading}
        className="disabled:opacity-25 text-base bg-eggshellWhite hover:bg-lightGray text-gray-800 px-4 border border-gray-400 rounded shadow my-2"
        onClick={() => {
          setLoading(true);
          locator();
        }}
      ...
      {loading ? <Loader /> : ""}
      {locError ? (
        <p>We could not locate your device. Check your privacy settings.</p>
      ) : null}
    </>
  );
}
